...
const realtorFees = (listing) => {
  //this is because im dumb and honestly forgot basic math in java so im just doing it here and creating pre made math things for it to use when calculating the realtor fees

  //i believe we learned this in a similar or this exact way last year but i found this method on stack overflow
  const price_string = listing.price.replace(/[$,]/g, '');
  //parses
  //const integerValue = parseInt(cleanedString, 10); 

  const math_2 = (price_string * 0.02);
  const math_25 = (price_string * 0.025);
  //test to make sure it works
  //console.log(math_25);
  
  return(price_string <= "450000" ? math_25 : price_string > "450000" ? math_2 : math_25 );
}
const fees = realtorFees(listings[6]);
console.log(fees);
 ...
 */
//WRITE YOUR CODE BELOW
// i guess i did it 2 ways that are nearly the same
...
console.log(listingAscendingly[0]);

//used as a reminder for what way to have it for descending or ascending
